...
try:
    ...
    apartments_folder = get_latest_timestamp_folder(args['s3_bucket'], apartments_prefix)
    ...
    viewing_metrics = viewings_df.agg(
        ...
        count(F.when(col("viewing_duration_seconds").isNotNull(), True)).alias("total_timed_viewings"),
    ...
    booking_metrics = bookings_df.agg(
        ...
        count(F.when(col("booking_status") == "confirmed", True)).alias("confirmed_bookings"),
    ...
    metrics_df = metrics_df.withColumn(
        ...
        F.when(col("total_viewings") > 0, 
               col("total_bookings") / col("total_viewings")
        ).otherwise(0)
    )
    ...
    metrics_df = metrics_df.withColumn(
        "wishlist_rate",
        F.when(col("total_viewings") > 0, 
               col("total_wishlisted") / col("total_viewings")
        ).otherwise(0)
    )
    
    metrics_df = metrics_df.withColumn(
        "avg_viewings_per_listing",
        F.when(col("active_listings") > 0, 
               col("total_viewings") / col("active_listings")
        ).otherwise(0)
    )
    ...
    redshift_table = f"{args['redshift_schema']}.presentation_metrics"
