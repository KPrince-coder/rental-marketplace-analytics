...
args = getResolvedOptions(
    sys.argv, ["JOB_NAME", "s3_bucket", "raw_prefix", "processed_prefix", "region"]
)
...
def process_user_viewings(dynamic_frame):
    ...
    # Add a default viewing duration of 0 if the columns don't exist
    df = df.withColumn("viewing_duration_seconds", lit(0))

    if "viewing_end_time" in df.columns and "viewed_at" in df.columns:
        df = df.withColumn(
            "viewing_duration_seconds",
            when(
                col("viewing_end_time").isNotNull() & col("viewed_at").isNotNull(),
...
def process_apartment_attributes(dynamic_frame):
    ...
    # Add a default price_per_sqft of 0
    df = df.withColumn("price_per_sqft", lit(0))

    # Try to use price_display if it exists, otherwise try price
    if "price_display" in df.columns and "square_feet" in df.columns:
        df = df.withColumn(
            "price_per_sqft",
            when(
                col("square_feet").isNotNull()
                & (col("square_feet") > 0)
                & col("price_display").isNotNull(),
